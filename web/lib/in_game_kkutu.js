...
$(document).ready(function(){
	...
	$data._soundList = [
		{ key: "k", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/k.mp3" },
		{ key: "lobby", value: "https://cdn.kkutu.xyz/media/"+$data.selectedLobbyBGM+"/LobbyBGM.mp3" },
		{ key: "jaqwi", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/JaqwiBGM.mp3" },
		{ key: "jaqwiF", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/JaqwiFastBGM.mp3" },
		{ key: "game_start", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/game_start.mp3" },
		{ key: "round_start", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/round_start.mp3" },
		{ key: "fail", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/fail.mp3" },
		{ key: "timeout", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/timeout.mp3" },
		{ key: "lvup", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/lvup.mp3" },
		{ key: "Al", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/Al.mp3" },
		{ key: "success", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/success.mp3" },
		{ key: "missing", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/missing.mp3" },
		{ key: "mission", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/mission.mp3" },
		{ key: "kung", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/kung.mp3" },
		{ key: "horr", value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/horr.mp3" },
	];
	for(i=0; i<=10; i++) $data._soundList.push(
		{ key: "T"+i, value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/T"+i+".mp3" },
		{ key: "K"+i, value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/K"+i+".mp3" },
		{ key: "As"+i, value: "https://cdn.kkutu.xyz/media/"+$data.selectedBGM+"/As"+i+".mp3" }
	);
	loadSounds($data._soundList, function(){
		processShop(connect);
	});
	delete $data._soundList;
	kkutu_alert = function(msg2, t) {
...
// 메뉴 버튼
	...
	$stage.menu.refresh.on('click', function(e){
		...
		// kkutu_alert("<i class='fas fa-sync-alt'></i> 접속자, 게임방 목록을 새로고침 하였습니다.<br>", 3000);
		send('refresh');
	...
	$stage.dialog.profileReport.on('click', function(e){
		if($data.guest) {
			kkutu_alert("손님 계정은 인 게임 신고 기능을 이용하실 수 없습니다. 우측 상단 로그인 버튼을 클릭하여 로그인해 주세요.");
			return;
		}
...
function applyOptions(opt){
	...
	$data.opts.istheme = $data.opts.tm === undefined ? true : $data.opts.tm;
	$data.selectedBGM = $data.opts.sb === undefined ? "start" : $data.opts.sb;
	$data.selectedLobbyBGM = $data.opts.sb === undefined ? "start" : $data.opts.so;
    ...
    switch (data.type) {
		...
		case 'welcome':
			...
			if($data.nick=="nonick") {
				...
				o.find('#newnick-ok').off('click').click(function(e) {
					...
					newnick = newnick !== undefined ? newnick.trim() : "";
					if (newnick.length<2) {
						kkutu_alert("닉네임은 두글자 이상으로 해주세요!");
						$(e.currentTarget).attr('disabled', false);
						return;
					} else if (newnick.length>15) {
						kkutu_alert("닉네임은 15글자 이하로 해주세요!");
						$(e.currentTarget).attr('disabled', false);
						return;
					} else if (newnick.length > 0 && !/^[가-힣a-zA-Z0-9][가-힣a-zA-Z0-9 ]*[가-힣a-zA-Z0-9]$/.exec(newnick)) {
						kkutu_alert("닉네임에는 한글/영문/숫자 및 공백만 사용 가능합니다!");
					...
					$.post("/newnick", obj, function(res){
						...
						send('refresh');
						kkutu_alert("닉네임 설정이 완료되었습니다! 궁금하신 점이 있으시다면 좌측 상단 물음표 아이콘을 클릭하여 도움말을 열어보세요.");
						updateUserList(true);
						o.hide();
						ov.hide();
					});
		...
		case 'error':
			...
			if(data.code == 401){
				...
				if(isNaN(blackEnds)) alert("[#444] 운영정책 위반으로 게임 이용이 정지되었습니다.\n\n차단 사유: " + i + "\n차단 기간: 영구 차단\n\n처벌과 관련된 문의는 공식카페를 이용하여 주시기 바랍니다.");
				else alert("[#444] 운영정책 위반으로 게임 이용이 일시적으로 정지되었습니다.\n\n차단 사유: " + i + "\n종료 일시: "+black.year+"년 "+black.month+"월 "+black.date+"일 "+black.hour+"시 까지\n\n처벌과 관련된 문의는 공식카페를 이용하여 주시기 바랍니다.");
				break;
	...
	$(".profile-head").empty().append($pi = $("<div>").addClass("moremi profile-moremi"))
		.append($("<div>").addClass("profile-head-item")
			...
			.append($("<div>").addClass("profile-title ellipse").html(o.profile.title || o.profile.name)
				.append($("<label>").addClass("profile-tag").html(" #" + o.id))
			)
		)
		.append($("<div>").addClass("profile-head-item")
...
function getLevelImage(score){
	...
	if(score < 0) return getImage("https://cdn.kkutu.xyz/img/kkutu/lv/gm.png");
	return $("<div>").css({
		'float': "left",
		'background-image': "url('https://cdn.kkutu.xyz/img/kkutu/lv/newlv.png')",
...
function setRoomHead($obj, room){
	...
	$obj.empty()
		.append(mobile?'':'<h5 style="position: absolute;left: 50%;transform: translateX(-50%);font-weight: bold;">알파끄투 - kkutu.xyz</h5>')
