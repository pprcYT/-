...
Server.get("/kpanel/daneo", function(req, res){
...
Server.get("/kpanel/user", function(req, res){
	...
	page(req, res, "kpanel/user");
});
Server.get("/kpanel/admin", function(req, res){
...
Server.get("/kpanel/gamsi", function(req, res){
	if(!checkUser(req, res)) return;
...
Server.get("/kpanel/users", function(req, res){
	if(!checkUser(req, res)) return;
...
Server.get("/kpanel/kkutudb/:word", function(req, res){
	if(!checkDaneo(req, res)) return;
...
Server.get("/kpanel/kkututheme", function(req, res){
	if(!checkDaneo(req, res)) return;
...
Server.get("/kpanel/shop/:key", function(req, res){
	if(!checkAdmin(req, res)) return;
...
Server.post("/kpanel/injeong", function(req, res){
	if(!checkDaneo(req, res)) return;
...
Server.post("/kpanel/kkutudel", function(req, res) {
	...
	list.forEach(function(item){
		...
		item = item.trim();
		if(!item.length) return;
		TABLE.remove([ '_id', item ]).on(function($res){
			JLog.info(`[ADMIN] ${res.toString()}`);
		});
	});
	if(!req.body.nof) res.sendStatus(200);	
});
Server.post("/kpanel/kkutudb/:word", function(req, res){
	if(!checkDaneo(req, res)) return;
...
Server.post("/kpanel/users", function(req, res){
	if(!checkUser(req, res)) return;
	if(req.body.pw != GLOBAL.PASS) return res.sendStatus(400);
...
Server.post("/kpanel/shop", function(req, res){
	if(!checkAdmin(req, res)) return;
	if(req.body.pw != GLOBAL.PASS) return res.sendStatus(400);
...
function checkAdmin(req, res){
	if(global.isPublic){
		if(req.session.profile){
			if(GLOBAL.MAIN.indexOf(req.session.profile.id) == -1){
...
function checkDaneo(req, res){
	if(global.isPublic){
		if(req.session.profile){
			if(GLOBAL.DANEO.indexOf(req.session.profile.id) == -1){
				req.session.daneo = false;
				return res.send({ error: 400 }), false;
			}
		}else{
			req.session.daneo = false;
...
function checkUser(req, res){
	if(global.isPublic){
		if(req.session.profile){
			if(GLOBAL.USER.indexOf(req.session.profile.id) == -1){
