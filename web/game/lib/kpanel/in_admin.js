...
(function(){
	...
	// 끄투 DB에 단어 삭제하기
	$("#dd-ok").on('click', function(e){
		$.post("/kpanel/kkutudel", {
			pw: $("#db-password").val(),
			lang: $("#db-lang").val(),
			list: $("#dd-list").val()
		}, function(res){
	...
	// 어인정 신청
		$("#injeong-go").on('click', function(e){
			$.get("/kpanel/injeong", function(res){
		...
		$("#injeong-apply").on('click', function(e){
			...
			$.post("/kpanel/injeong", { list: JSON.stringify({ list: list }), pw: $("#db-password").val() }, function(res){
				alert(res);
			});
		});
	...
	// 상점 DB 다루기
		$("#shop-go").on('click', function(e){
			$.get("/kpanel/shop/" + $("#shop-word").val(), function(res){
	...
	// 끄투에서의 인기 단어
		$("#kpw-query").on('click', function(e){
			...
			$.get("/kpanel/kkutuhot", function(res){
		...
		$("#kpw-flush").on('click', function(e){
			$.post("/kpanel/kkutuhot", { pw: $("#db-password").val() }, function(res){
				alert(res);
			});
		});
	});
